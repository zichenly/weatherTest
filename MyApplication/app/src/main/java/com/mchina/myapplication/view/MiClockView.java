package com.mchina.myapplication.view;
...
import com.mchina.myapplication.R;
import com.mchina.myapplication.utils.DensityUtils;
...
public class MiClockView extends View {
    ...
    public MiClockView(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.MiClockView, 0, 0);
        mDarkColor = typedArray.getColor(R.styleable.MiClockView_clock_darkColor, Color.parseColor("#80ffffff"));
        mLightColor = typedArray.getColor(R.styleable.MiClockView_clock_lightColor, Color.parseColor("#ffffff"));
        mBackgroundColor = typedArray.getColor(R.styleable.MiClockView_clock_backgroundColor, Color.parseColor("#237EAD"));
        mTextSize = typedArray.getDimension(R.styleable.MiClockView_clock_textSize, DensityUtils.sp2px(context, 14));
        typedArray.recycle();
        initPaint();
    }
...
//        super.onMeasure(widthMeasureSpec, heightMeasureSpec);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mSweepGradient = new SweepGradient(w / 2f, h / 2f, new int[]{mDarkColor, mLightColor}, new float[]{0.75f, 1});
    }
...
//        mCanvas.translate(mCanvasTranslateX, mCanvasTranslateY);
        ...
        for (int i = 0; i < 200; i++) {
            mCanvas.drawLine(getWidth() / 2f, mPaddingTop + mScaleLineLen + mTextRect.height() / 2f,
                    getWidth() / 2f, mPaddingTop + mScaleLineLen * 2 + mTextRect.height() / 2f, mScaleLinePaint);
...
//        mCanvas.translate(mCanvasTranslateX * 1.2f, mCanvasTranslateY * 1.2f);
        ...
        mHourHandPath.quadTo(getWidth() / 2f, offset + 0.46f * mRadius, getWidth() / 2f + 0.009f * mRadius, offset + 0.48f * mRadius);
